...
Let's dive into profiling a Wasm application using WAMR JIT and linux perf.

1. Before profiling, recompile WAMR with the LLVM JIT compilation option:

```bash
$ cmake -S . -B bulid -DWAMR_BUILD_JIT=1
```
...
$ perf record -k mono -g --output=perf.data.raw -- iwasm --perf-profile <wasm>
